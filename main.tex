...
Our contribution is a post-hoc variance-weighted graph smoother for location-specific scales when a target location has limited recent data. Locations are represented as nodes in a graph, with edges connecting related locations. We smooth log-scales over this graph, giving greater weight to estimates with lower estimated variance. A separate calibration step adjusts the interval widths toward the coverage target using a split-conformal quantile rule~\cite{lei2018distributionfree}. We evaluate how Winkler scores change as the target location's available data decreases while other locations retain their available data.
...
We split data chronologically into training (60\%), validation (10\%), tuning (10\%), calibration (10\%), and testing (10\%). Each reference variance estimate uses 200 sampled windows. For each target history and forecast horizon, we average tuning Winkler scores equally across locations and select the least smoothing among 15 settings within 1\% of the lowest mean score. Interval radii remain fixed during testing.
...
Tuning uses an approximate 60/20/20 split for older scale estimates, calibration and recent scales, and candidate scoring, with 12-slot boundary offsets. Among observations available before scoring, the middle portion's latest 48 slots estimate recent scales and earlier slots calibrate intervals. With $\tau_j=1$, each target sensor uses 0, 12, 24, or 48 recent five-minute slots, or its full available data. Other sensors retain their full available data.

For testing, full data combines tuning data with the latest 48 calibration slots available before testing begins. Limited-data targets use their assigned recent slots. Earlier calibration data from other sensors determines interval multipliers. Evaluation requires an observed target speed and 12 observed input measurements at the target sensor.
...
\end{cases}
\end{equation}
Tuning uses the first 32 dates for older scale estimates and the final 21 for scoring. Among intervening observations available before scoring, the latest 14 dates estimate recent scales and earlier dates calibrate intervals. For testing, the earlier 78 calibration dates from states $j\ne i$ determine $q_i$. Testing covers 92 daily forecasts per state from January 26 to April 26, 2024.
...
With limited target data, Graph Smoothing gives mean interval widths of 60.5--64.0 admissions, compared with 74.5 for Global. Although narrower than Global's, these intervals have mean empirical coverage of 96.7--98.7\%, above the nominal 90\% target. This application demonstrates the use of the same scale-sharing method around hospital-admission forecasts.

\subsection{Discussion and Limitations}

Graph Smoothing's gains over Global are clearer in Los Angeles and hospital admissions, while Bay Area scores remain close to Global. Gains are clearest with limited target data. With full data, Graph Smoothing and Local have equal or nearly equal mean scores across all three datasets.
