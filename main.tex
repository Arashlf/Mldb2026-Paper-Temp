...
Let $\mathcal E_j$ contain the usable estimation times for location $j$, with $n_j=|\mathcal E_j|$. We use scaled absolute forecast errors
\begin{equation}
...
\end{cases}
\end{equation}
The training-period standard deviation $\sigma_{j,\mathrm{train}}$ makes forecast errors comparable across states and remains available when recent interval-estimation data are absent.
...
\end{equation}
The percentile summarizes the magnitude of past forecast errors, while the floor permits logarithms.
...
Let $A=(a_{jk})$ be the symmetric weighted adjacency matrix, $E=\{\{j,k\}:j<k,\ a_{jk}>0\}$, and $\mathcal O=\{j\in V:w_j>0\}$. For $\lambda\geq0$, we use graph Tikhonov regularization~\cite{shuman2013emerging}, with inverse-variance weights on the fit to the starting log-scales,
\begin{equation}
\begin{aligned}
...
\end{equation}
The weighted least-squares term retains more precise estimates, while the Laplacian regularization term penalizes differences between connected log-scales. This quadratic choice gives a convex objective and a linear system. With $W=\operatorname{diag}(w_j)$ and graph Laplacian $L=\operatorname{diag}(A\mathbf 1)-A$, its first-order condition gives
\begin{equation}
...
At $\lambda=0$, $\tilde s_j=s_j$ for $j\in\mathcal O$ and $\tilde s_j=s_{-i}$ otherwise. For $\lambda>0$, we solve each connected component containing a location in $\mathcal O$ separately. Within such a component, a location without a usable estimate receives a harmonic extension, meaning its log-scale is a weighted average of its neighbors' log-scales. Components containing no location in $\mathcal O$ use $s_{-i}$.
...
The three approaches differ in their scales. Set $b_j=1$ for Global, $b_j=s_j$ for Local, and $b_j=\tilde s_j$ for Graph Smoothing. The ratios $d_{jt}/b_j$ are nonconformity scores, measuring forecast errors relative to the chosen scale. Each approach calculates its own multiplier and radius,
...
Global therefore uses a pooled quantile of scaled absolute forecast errors, while Local and Graph Smoothing additionally normalize by their location-specific scales. The multiplier $q_i$ adjusts interval size toward the 90\% target, and $\tau_i$ returns the radius to the original units. Local isolates the effect of smoothing under the same calibration procedure. Because locations differ and conditions change over time, empirical coverage may differ from 90\%. We measure it on the target location's test data.
