...
\end{equation}
The normalization factor $\tau_j$ is fixed before interval estimation and remains available at cold start. Setting $\tau_j=1$ retains the original units.

For sorted values $x_{(1)}\leq\cdots\leq x_{(n)}$, define $Q_{1-\alpha}(x)=x_{(1+\lceil(1-\alpha)(n-1)\rceil)}$. A floor $\epsilon=10^{-6}$ permits logarithms. The pooled fallback and starting scales are
\begin{equation}
\begin{aligned}
...
\end{equation}
Weighted least squares retains precise estimates, while Laplacian regularization penalizes differences between connected log-scales. The convex quadratic objective yields a linear system, with $W=\operatorname{diag}(w_j)$ and graph Laplacian $L=\operatorname{diag}(A\mathbf 1)-A$,
\begin{equation}
(W+\lambda L)\hat{\mathbf z}=W\log\mathbf s,
\qquad \tilde s_j=e^{\hat z_j}>0.
\label{eq:smoothing-system}
\end{equation}
Log-space smoothing ensures positivity and penalizes relative differences through $\hat z_j-\hat z_k=\log(\tilde s_j/\tilde s_k)$.
...
The multiplier $q_i$ adjusts interval size toward coverage $1-\alpha$, and $\tau_i$ restores the original units. Local isolates the effect of smoothing under the same calibration procedure. Heterogeneity across nodes and temporal dependence can shift coverage from the target. We therefore report empirical coverage on test data.
...
We use $\tau_j=1$ for all sensors and estimate uncertainty for one target sensor at a time using predicted and observed speeds from its latest 0, 12, 24, or 48 five-minute slots, or its full available data for interval estimation. Other sensors retain their full available data. We repeat each case for every sensor.
...
Each node represents a state. We evaluate seven-day-ahead forecasts of hospital admissions, using $\tau_j=\sigma_{j,\mathrm{train}}$, the state's training-period standard deviation, for normalization. This factor remains available at cold start. For nonnegative admission counts, the lower interval endpoint is $\max(0,\hat y_{it}-r_i)$. A forecast error becomes available for interval estimation once its corresponding admission count is observed. The experiment uses a retrospective data extract with observations treated as available on their recorded dates.
