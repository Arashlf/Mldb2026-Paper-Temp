...
\end{equation}
From an older period of length $T$, sample $B$ consecutive windows of length $\ell=\min(m,\lfloor T/2\rfloor)$ for reference length $m\leq T$, with starts drawn uniformly with replacement. For window $b$, let $u_{jb}$ and $n_{jb}$ denote the log-scale and usable count. Then
\begin{equation}
\begin{aligned}
...
At $\lambda=0$, $\tilde s_j=s_j$ for $j\in\mathcal O$ and $\tilde s_j=s_{-i}$ otherwise. For $\lambda>0$, we solve components intersecting $\mathcal O$ separately. Within them, nodes outside $\mathcal O$ receive a harmonic extension, a weighted average of neighboring log-scales. Components disjoint from $\mathcal O$ use $s_{-i}$.
...
\end{equation}
Matrices are restricted to components intersecting $\mathcal O$, with $\rho(0)=1$. Smaller ratios give stronger sharing. Tuning selects one ratio, and each target case's weights determine $\lambda$. Grid value zero approximates the component-wise shared-scale limit using a large finite $\lambda$.
