...
Let $V=\{1,\ldots,p\}$ index sensors or states, with observations $y_{jt}$ and forecasts $\hat y_{jt}$ at location $j\in V$ and time $t$. For target location $i\in V$, radius $r_i$ defines
...
For hospital admissions, the lower endpoint is $\max(0,\hat y_{it}-r_i)$. We target 90\% coverage separately for each forecast horizon and amount of target data.

\subsection{Starting Scales}

For usable estimation times $\mathcal E_j$ and count $n_j=|\mathcal E_j|$, define scaled absolute forecast errors
\begin{equation}
...
\end{cases}
\end{equation}
The training-period standard deviation $\sigma_{j,\mathrm{train}}$ allows cross-state comparisons and remains available at cold start.

For sorted values $x_{(1)}\leq\cdots\leq x_{(n)}$, define $Q_{.9}(x)=x_{(1+\lceil.9(n-1)\rceil)}$. A floor $\epsilon=10^{-6}$ permits logarithms. The pooled fallback and starting scales are
\begin{equation}
\begin{aligned}
...
Let $\mathcal C$ index usable calibration pairs and $\mathcal C_i=\{(j,t)\in\mathcal C:j\ne i\}$. For $N$ sorted scores, the split-conformal rule~\cite{lei2018distributionfree} gives $Q_{\mathrm{cal}}(x)=x_{(\lceil0.90(N+1)\rceil)}$, with $x_{(N+1)}=+\infty$.
...
The multiplier $q_i$ adjusts interval size toward 90\% coverage, and $\tau_i$ restores the original units. Local isolates the effect of smoothing under the same calibration procedure. Spatial heterogeneity and temporal change can shift coverage from the target. We therefore report empirical coverage on test data.
...
We evaluate seven-day-ahead forecasts of hospital admissions. The training period provides each state's normalization factor. A forecast error becomes available for interval estimation once its corresponding admission count is observed. The experiment uses a retrospective data extract with observations treated as available on their recorded dates.
